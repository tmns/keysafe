    ...
    >
      <div className="bg-white rounded shadow-md h-64 mx-2 md:mx-0 w-full md:w-1/3">
        ...
        <div className="text-center text-red-600 text-bold pt-2 px-5">
          Are you sure you want to delete this key?<br /> It will be permanently
          removed!
          <div className="bg-blue px-4 pt-8 text-center">
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await axios.delete(
                    `/api/groups/key/${props.groupId}/${props.keyId}`
                  );

                  const updatedKeys = props.keys.filter(
                    key => key._id != props.keyId
                  );
